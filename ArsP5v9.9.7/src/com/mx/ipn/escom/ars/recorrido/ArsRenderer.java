...
public class ArsRenderer implements GLSurfaceView.Renderer {
	...
	private ArsActivity actividad;
	private ReconocerActivity actividad2;	
	
	static final int SHOW = 0;
	
	public Boolean banderaFinalizar=false;

	public void prueba(String audio) {
		drawScene();						
		/**
		 * Cambia el audio dependiendo la imagen que se observe
		 */
		audio = audio.concat(".mp3");
		if (actividad.getArchivo() != null) {
			if (!actividad.getArchivo().equals(audio)) {
				actividad.stopAudio();
				actividad.setAudio(audio);				
				actividad.getFatso().setFps(10);
				actividad.getFatso().play(1,15,false,actividad.getFatso());				
//				actividad.openOptionsMenu();
			}
		} else {
			actividad.setAudio(audio);
		}		
	...
	public void prueba2(String cad) {
		drawScene();						
		
		if(banderaFinalizar==false){
		Log.v("ArsRenderer", "LLAMANDO ACTIVIDAD");
		actividad2.finish();		
		Intent intent = new Intent(context,MenuMuseoActivity.class);
		intent.putExtra("MuseoID", cad);
		intent.putExtra("Action", "Descarga");
	...
	public ArsRenderer(Scene $scene, ArsActivity a) {
		this.actividad = a;
	...
	public ArsRenderer(Scene $scene, ReconocerActivity a) {
		this.actividad2 = a;
...
//		Log.i(Min3d.TAG, "DrawFrame");
	...
	/**
	 * Accessor to the GL object, in case anything outside this class wants to
	...
	protected void drawSetup() {
		...
		GLU.gluLookAt(_gl, _scene.camera().position.x,
				_scene.camera().position.y, _scene.camera().position.z,
				_scene.camera().target.x, _scene.camera().target.y,
				_scene.camera().target.z, _scene.camera().upAxis.x,
				_scene.camera().upAxis.y, _scene.camera().upAxis.z);

		// Background color

		if (_scene.backgroundColor().isDirty()
				&& _scene.backgroundTransparent() == false) {
			_gl.glClearColor((float) _scene.backgroundColor().r() / 255f,
					(float) _scene.backgroundColor().g() / 255f, (float) _scene
							.backgroundColor().b() / 255f, (float) _scene
...
//		_gl.glEnableClientState(GL10.GL_VERTEX_ARRAY);
	...
	protected void drawSetupLights() {
		...
		for (int glIndex = 0; glIndex < NUM_GLLIGHTS; glIndex++) {
			if (_scene.lights().glIndexEnabledDirty()[glIndex] == true) {
				...
				_scene.lights().glIndexEnabledDirty()[glIndex] = false; // clear
																		// dirtyflag
			}
		}
		...
		for (int i = 0; i < lights.length; i++) {
			...
			if (light.isDirty()) // .. something has changed
			{
				...
				if (light.attenuation().isDirty()) {
					_gl.glLightf(glLightId, GL10.GL_CONSTANT_ATTENUATION, light
							.attenuation().getX());
					_gl.glLightf(glLightId, GL10.GL_LINEAR_ATTENUATION, light
							.attenuation().getY());
					_gl.glLightf(glLightId, GL10.GL_QUADRATIC_ATTENUATION,
							light.attenuation().getZ());
				}
	...
	protected void drawObject(Object3d $o) {
		...
		if ($o.hasNormals() && $o.normalsEnabled()) {
			...
			_gl.glNormalPointer(GL10.GL_FLOAT, 0, $o.vertices().normals()
					.buffer());
		 ...
		 * _scratchIntBuffer.position(0); _gl.glGetIntegerv(GL10.GL_LIGHTING,
		 * _scratchIntBuffer); if (useLighting != _scratchIntBuffer.get(0)) { if
		 * (useLighting == 1) { _gl.glEnable(GL10.GL_LIGHTING); } else {
		 * _gl.glDisable(GL10.GL_LIGHTING); } }
		 */

		boolean useLighting = (_scene.lightingEnabled() && $o.hasNormals()
...
//			_gl.glColorPointer(4, GL10.GL_UNSIGNED_BYTE, 0, $o.vertices()
//					.colors().buffer());
//			_gl.glEnableClientState(GL10.GL_COLOR_ARRAY);
//		} else {
			_gl.glColor4f((float) $o.defaultColor().r / 255f,
					(float) $o.defaultColor().g / 255f,
					(float) $o.defaultColor().b / 255f,
					(float) $o.defaultColor().a / 1f);
...
//		}
		...
		_gl.glVertexPointer(3, GL10.GL_FLOAT, 0, $o.vertices().points()
				.buffer());
		...
		if (!$o.ignoreFaces()) {
			...
			if (!$o.faces().renderSubsetEnabled()) {
				...
				pos = $o.faces().renderSubsetStartIndex()
			...
			_gl.glDrawElements($o.renderTypeInt(), len
					* FacesBufferedList.PROPERTIES_PER_ELEMENT,
					GL10.GL_UNSIGNED_SHORT, $o.faces().buffer());
		} else {
	...
	private void drawObject_textures(Object3d $o) {
		...
		for (int i = 0; i < RenderCaps.maxTextureUnits(); i++) {
			...
			if ($o.hasUvs() && $o.texturesEnabled()) {
				...
				if (textureVo != null) {
					// activate texture
					int glId = _textureManager
							.getGlTextureId(textureVo.textureId);
					_gl.glBindTexture(GL10.GL_TEXTURE_2D, glId);
					_gl.glEnable(GL10.GL_TEXTURE_2D);
					_gl.glEnableClientState(GL10.GL_TEXTURE_COORD_ARRAY);

					int minFilterType = _textureManager
							.hasMipMap(textureVo.textureId) ? GL10.GL_LINEAR_MIPMAP_NEAREST
							: GL10.GL_NEAREST;
					_gl.glTexParameterf(GL10.GL_TEXTURE_2D,
							GL10.GL_TEXTURE_MIN_FILTER, minFilterType);
					_gl.glTexParameterf(GL10.GL_TEXTURE_2D,
							GL10.GL_TEXTURE_MAG_FILTER, GL10.GL_LINEAR); // (OpenGL
																			// default)

					// do texture environment settings/////////////////////////////////////////////////////////////////////////
//					 for (int j = 0; j < textureVo.textureEnvs.size(); j++)
//					 {
//					 _gl.glTexEnvx(GL10.GL_TEXTURE_ENV,
//					 textureVo.textureEnvs.get(j).pname,
//					 textureVo.textureEnvs.get(j).param);
//					 }

					// texture wrapping settings
					_gl.glTexParameterx(GL10.GL_TEXTURE_2D,
							GL10.GL_TEXTURE_WRAP_S,
							(textureVo.repeatU ? GL10.GL_REPEAT
									: GL10.GL_CLAMP_TO_EDGE));
					_gl.glTexParameterx(GL10.GL_TEXTURE_2D,
							GL10.GL_TEXTURE_WRAP_T,
							(textureVo.repeatV ? GL10.GL_REPEAT
									: GL10.GL_CLAMP_TO_EDGE));
					...
					if (textureVo.offsetU != 0 || textureVo.offsetV != 0) {
						...
						_gl.glLoadIdentity();
						_gl.glTranslatef(textureVo.offsetU, textureVo.offsetV,
								0);
						_gl.glMatrixMode(GL10.GL_MODELVIEW); // .. restore
																// matrixmode
					}
				} else {
//					Log.v("prueba", "texturevo=null");
					_gl.glBindTexture(GL10.GL_TEXTURE_2D, 0);
					_gl.glDisable(GL10.GL_TEXTURE_2D);
					_gl.glDisableClientState(GL10.GL_TEXTURE_COORD_ARRAY);
				}
			} else {
//				Log.v("prueba", "textures no enabled");
				_gl.glBindTexture(GL10.GL_TEXTURE_2D, 0);
				_gl.glDisable(GL10.GL_TEXTURE_2D);
				_gl.glDisableClientState(GL10.GL_TEXTURE_COORD_ARRAY);
			}
																				 ...
																				 */
	{
		...
		int[] a = new int[1];
		_gl.glGenTextures(1, a, 0); // create a 'texture name' and put it in
									// array element 0
		glTextureId = a[0];
		_gl.glBindTexture(GL10.GL_TEXTURE_2D, glTextureId);

		if ($generateMipMap && _gl instanceof GL11) {
			_gl.glTexParameterf(GL11.GL_TEXTURE_2D, GL11.GL_GENERATE_MIPMAP,
					GL11.GL_TRUE);
		} else {
			_gl.glTexParameterf(GL11.GL_TEXTURE_2D, GL11.GL_GENERATE_MIPMAP,
					GL11.GL_FALSE);
	...
	private void doFps() {
		...
		if (delta >= FRAMERATE_SAMPLEINTERVAL_MS) {
			...
			_activityManager.getMemoryInfo(_memoryInfo);
			Log.v(Min3d.TAG,
					"FPS: " + Math.round(_fps) + ", availMem: "
							+ Math.round(_memoryInfo.availMem / 1048576) + "MB");
	...
	private void reset() {
		...
		_gl.glTexParameterf(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_MAG_FILTER,
				GL10.GL_LINEAR); // (is OpenGL default)
		...
		//
		// Scene object init only happens here, when we get GL for the first
		// time
		//
	}
}
...
// _gl.glMaterialfv(GL10.GL_FRONT_AND_BACK, GL10.GL_EMISSION,
// c.toFloatBuffer());
// /Color4 c = new Color4(0,128,0, 255);
// /_gl.glMaterialfv(GL10.GL_FRONT_AND_BACK, GL10.GL_SPECULAR,
// c.toFloatBuffer());
// Color4 c = new Color4(128,128,128,128);
// _gl.glMaterialfv(GL10.GL_FRONT_AND_BACK, GL10.GL_SHININESS,
// c.toFloatBuffer());
