...
public class ReconocerActivity extends Activity implements ISceneController {
	...
	private class InitQCARTask extends AsyncTask<Void, Integer, Boolean> {
		...
		protected Boolean doInBackground(Void... params) {
			...
			synchronized (mShutdownLock) {
				QCAR.setInitParameters(ReconocerActivity.this, mQCARFlags);
	...
	private class LoadTrackerTask extends AsyncTask<Void, Integer, Boolean> {
		protected Boolean doInBackground(Void... params) {
			synchronized (mShutdownLock) {
				String path = Environment.getExternalStorageDirectory() + "/Ars/config/museos.xml";
...
//		museoId=datos.getString("MuseoID");
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		} else if (item.getTitle().equals("Cambiar animaci�n")) {
//			fatso.play(1, 11, true);
...
//			fatso.play(44,79,true);
	...
	public void setAudio(String audioNombre) {
		try {			
			mp.reset();
			descriptor = getAssets().openFd(audioNombre);
