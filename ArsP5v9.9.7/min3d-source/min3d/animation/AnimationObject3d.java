...
public class AnimationObject3d extends Object3d {
	...
	private Integer currentFrameEndIndex;
	...
	private String currentFrameEndName;
	private Integer loopStartIndex;
	private boolean loop = false;
	private AnimationObject3d animacion=null;
	...
	public void play(int frame) {
		loopStartIndex = currentFrameIndex = frame;
		currentFrameName = frames[frame].getName();
		currentFrameEndIndex=numFrames-1;
		startTime = System.currentTimeMillis();
		isPlaying = true;
	}	
	
	public void play(int frame, boolean loop) {
		this.loop = loop;
		play(frame);
	}
	
	public void play(int frame,int end,boolean loop) {
		this.loop=loop;
		loopStartIndex = currentFrameIndex = frame;
		currentFrameEndIndex=end;
		currentFrameName = frames[frame].getName();
		currentFrameEndName=frames[end].getName();
		startTime = System.currentTimeMillis();
		isPlaying = true;		
	}
	
	public void play(int frame,int end,boolean loop,AnimationObject3d model) {
		this.loop=loop;
		loopStartIndex = currentFrameIndex = frame;
		currentFrameEndIndex=end;
		currentFrameName = frames[frame].getName();
	...
	public void update() {
		if (!isPlaying || !updateVertices)
			return;
//		Log.i(Min3d.TAG, currentFrameIndex.toString());
		...
		if(currentFrameIndex==currentFrameEndIndex){
			if(!loop){
				stop();
			if(!animacion.equals(null)){
				animacion.setFps(2);
				animacion.play(17,43,true);
			}
			}
			else
