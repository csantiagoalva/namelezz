...
public class ManagedLightList 
{
	...
	public void reset() 
	{
		...
		for (int i = 0; i < ArsRenderer.NUM_GLLIGHTS; i++) {
			_availGlIndices.add(i);
		}

		_lightToGlIndex = new HashMap<Light, Integer>();

		_glIndexEnabled = new boolean[ArsRenderer.NUM_GLLIGHTS];
		_glIndexEnabledDirty = new boolean[ArsRenderer.NUM_GLLIGHTS];
		for (int i = 0; i < ArsRenderer.NUM_GLLIGHTS; i++) {
	...
	public boolean add(Light $light) 
	{
		...
		if (_lights.size() > ArsRenderer.NUM_GLLIGHTS)
